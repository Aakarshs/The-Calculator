...
export default class Calculator extends React.Component {
    ...
    update() {
        ...
        this.saveHistory();
        try {
            // if (isNaN(Number(this.state.text[this.state.text.length - 1])) === false) {
            if (isNaN(Number(this.state.text[this.state.text.length - 1])) === false ||
                ...
                this.state.text[this.state.text.length - 1] === '*') {
                var sol = math.eval(this.state.text);
                this.setState({ sol_history: sol })
            }
        }
        catch (err) {
            var sol = this.state.sol_history;
        }
        this.setState({ sol_history: sol })
        var x = this.state.videos.slice();
        var y = this.state.text;
                        ...
                        <View>
                            <View style={{
                                ...
                                <TouchableOpacity
                                    delayPressIn={0}
                                    delayPressOut={0}
                                    onPress={() => { requestAnimationFrame(() => { this.carouselAnim(); this.spring(14); this.send_to_back() }) }}>
                        ...
                        <View>
                            <View style={{
                                ...
                                <Animated.View style={{
                                    ...
                                    <TouchableHighlight
                                        onPress={() => { this.setState({ text: this.state.text + 'sin(' }, () => { this.update() }) }}
                                        style={styles.button}>
                                        <Text style={{
                                            lineHeight: 93,
                                            fontFamily: 'ample5',
                                    ...
                                    <TouchableHighlight
                                        onPress={() => { this.setState({ text: this.state.text + 'cos(' }, () => { this.update() }) }}
                                        style={styles.button}>
                                        <Text style={{
                                            lineHeight: 93,
                                            fontFamily: 'ample5',
